...
///
/// Before this, video played through libmpv with no session attached:
/// starting a stream and pulling down the shade showed nothing, so the only
/// way to pause was to return to the app, and Android was free to kill the
/// process the moment it was backgrounded.
///
...
/// calls. One handler covers video (the only source registered with the
/// coordinator here) and anything else that registers with it later, rather
/// than each source growing its own session.
    ...
    try {
      _handler = await AudioService.init(
        ...
        config: const AudioServiceConfig(
          androidNotificationChannelId: 'com.mediahub.playtorriomov.audio',
          ...
          // paused. Dropping it on pause makes a paused stream unresumable
          // from the shade, which is the state people leave video in most.
