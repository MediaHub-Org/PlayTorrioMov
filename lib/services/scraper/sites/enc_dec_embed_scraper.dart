...
/// The shared pipeline behind the vidfast.vc and vidup.to scrapers.
///
/// The two were separate 200-line files that differed in exactly three
/// strings: the embed domain, the slug in the enc-dec.app endpoint pair,
/// and the label shown on a resolved source. Everything else -- the token
/// scrape off the embed page, the encrypt/decrypt round trip, the parallel
/// server fan-out, the quality parsing -- was line-for-line identical, so a
/// fix to the pipeline had to be made twice or it silently only half
/// applied.
///
/// Subclasses supply the three strings and nothing else.
abstract class EncDecEmbedScraper extends StreamScraper {
  @override
  String get name => 'PlayTorrioHTTP';

  /// The embed host, e.g. `https://vidfast.vc`. Used for the page request
  /// and for the Referer/Origin the API expects.
  String get domain;

  /// The suffix on the enc-dec.app endpoint pair, e.g. `vidfast` for
  /// `/enc-vidfast` and `/dec-vidfast`.
  ...
  Stream<StreamSource> scrapeStream({
    ...
    try {
      ...
      if (tmdbId == null) {
        if (kDebugMode) debugPrint('[$displayName] Could not resolve TMDB ID for "$title"');
        return;
      }
      ...
      try {
        ...
        if (pageRes.statusCode != 200) {
          if (kDebugMode) {
            debugPrint('[$displayName] Embed page returned ${pageRes.statusCode}');
          }
          return;
        }
        ...
        if (tokenMatch == null || tokenMatch.group(1) == null) {
          if (kDebugMode) {
            debugPrint('[$displayName] No token match found in embed HTML');
          }
          return;
        }
        ...
        // Query the encrypt endpoint
        final encRes = await client.get(
          Uri.parse('$_apiBase/enc-$encDecSlug?text=${Uri.encodeComponent(rawToken)}'),
        ...
        final decServersRes = await client.post(
          Uri.parse('$_apiBase/dec-$encDecSlug'),
        ...
        for (final item in resolved) {
          ...
          yield StreamSource(
            ...
            title: '$displayName · $srvName · $qualityLabel',
            description: '$displayName Multi-CDN Stream · $qualityLabel',
            url: streamTargetUrl,
            headers: headers,
            behaviorHints: {
              'notWebReady': false,
      ...
      if (kDebugMode) debugPrint('[$displayName] scrapeStream error: $e');
    }
  }
}
