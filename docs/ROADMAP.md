...
| 5  | Two different meanings of "desktop" | Mostly closed: the genuine tier checks that were still raw widths (`WatchScreen._isDesktop`, Movies/Series' `isCompact`) now use `AppBreakpoints`. What is deliberately *not* migrated: the grid column ramps (1200/900/650/600/420) are finer-grained than the three nav tiers, and IPTV's portal browser measures content width minus its sidebar rather than the screen — forcing either onto `AppBreakpoints` would be wrong, not tidier. What is genuinely open is that `AnimeDetailsPage._isDesktop()` and `IptvHeroCarousel._isDesktop()` test the *platform* (`defaultTargetPlatform`), while every other `_isDesktop()` tests the *width* — so a narrow window on macOS is "desktop" to two widgets and "mobile" to the rest. Picking one needs a decision about which behaviour is wanted, not a rename. |
